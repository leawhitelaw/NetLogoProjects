...
This model will run the simulation 30 times and output the results of the agent energy or resource energy to a CSV file depending on what is commented out in the code tab. 
...
Change what is and is not commented out in the 'calculate-totals' and 'output-csv' functions to get the desired results. Pay attention to lists that need to be initialised in zero-counter or setup to avoid getting an error message.

To change the number of times the simulation is run, replace the number in the following line in 'go': iteration-counter > 30 

To only record results where an institution is established or not established, uncomment the following line if 'InstitutionExists? = false [' in 'go' true = institution established, false = institution not established.
